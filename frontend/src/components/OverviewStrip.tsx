...
export function OverviewStrip() {
  const { satellites, alerts, positions } = useAppStore();
  const activeObjects = satellites.filter((satellite) => satellite.hoursSinceEpoch < 48).length;
  const propagatedObjects = Object.keys(positions).length;
  ...
  return <section className="overview-strip panel" aria-label="Orbital overview">
    <div className="overview-card overview-objects">
      <div className="overview-heading"><span>AT A GLANCE</span><span className="mini-spark">⌁</span></div>
      <div className="overview-main-number">{satellites.length || "—"}<span> objects</span></div>
      <div className="overview-foot"><span className="status-led" />{activeObjects || "—"} recent <i /> {propagatedObjects ? `${propagatedObjects} screened` : "not screened"}</div>
    </div>
    <div className="overview-card">
      <div className="overview-heading"><span>CLOSE-APPROACH SUMMARY</span><span className="cyan-text">24H</span></div>
      <div className="summary-grid"><div><strong>{alerts.length || "—"}</strong><small>FLAGGED</small></div><div className="danger-number"><strong>{critical || "—"}</strong><small>HIGH RISK</small></div><div><strong>{propagatedObjects || "—"}</strong><small>SCREENED</small></div></div>
    </div>
    <div className="overview-card risk-distribution">
      <div className="overview-heading"><span>RISK MIX</span><span className="mini-spark">◔</span></div>
